...
func TestGetFullOwnerConversationIDsFreshDeviceWithoutPaginationKeepsLegacyFullIDs(t *testing.T) {
	...
	resp, err := srv.GetFullOwnerConversationIDs(context.Background(), &pbconversation.GetFullOwnerConversationIDsReq{
...
func TestGetFullOwnerConversationIDsReadInactiveKeepsLegacyWhenFlagOff(t *testing.T) {
	...
	srv := &conversationServer{
		conversationDatabase: &fakeConversationDatabase{conversationIDs: ids},
		msgRpcClient: fakeMessageClient{seqs: map[string]*pbmsg.FullSyncSeqs{
	...
	if resp.Version != hashutil.IdHash(ids) {
		t.Fatalf("expected legacy hash %d, got %d", hashutil.IdHash(ids), resp.Version)
	}
}
...
func TestGetFullOwnerConversationIDsReadInactiveFiltersReadInactiveConversations(t *testing.T) {
	...
	if resp.Version != hashutil.IdHash(filteredIDs) {
		t.Fatalf("expected filtered hash %d, got %d", hashutil.IdHash(filteredIDs), resp.Version)
	}
}
...
func BenchmarkGetFullOwnerConversationIDsLegacyFullIDs(b *testing.B) {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		resp, err := srv.GetFullOwnerConversationIDs(context.Background(), req)
		if err != nil {
			b.Fatal(err)
		}
...
func BenchmarkGetFullOwnerConversationIDsReadInactiveFilteredIDs(b *testing.B) {
	...
	srv := &conversationServer{
		conversationDatabase: &fakeConversationDatabase{conversationIDs: ids},
		msgRpcClient:         fakeMessageClient{seqs: seqs},
	}
	req := &pbconversation.GetFullOwnerConversationIDsReq{
		UserID: "customer-service",
		IdHash: 0,
...
func (f *fakeConversationDatabase) CreateGroupChatConversation(context.Context, string, []string) error {
	return nil
}
